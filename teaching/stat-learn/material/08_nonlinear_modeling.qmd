...
Notice that the p-values are the same, and in fact the square of the $t$-statistics are equivalent to the F-statistics from the `anova()` function; for example:
...
The generic `plot()` function recognizes that `gam.m3` is an object of class `Gam`, and invokes the appropriate `plot.Gam()` method. Conveniently, even though `gam1` is not of class `Gam` but rather of class `lm`, we can {\em
